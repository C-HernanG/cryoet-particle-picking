...
EXP4 studies whether the remaining variability after fine-tuning is primarily a tomogram problem or a prompt problem. More specifically, it tests the acquisition-to-model rotation sensitivity described in Section~\ref{sec:rotation-sensitivity}.

\subsection{Question and hypothesis}
The question is whether a fine-tuned detector is effectively invariant to prompt orientation and prompt identity once the large domain-shift issue has been solved. The hypothesis is twofold: family-level averages will remain high after fine-tuning, but prompt-level differences will still survive and will correlate more strongly with prompt geometry or embedding descriptors than with SNR alone.

\subsection{Implementation and controls}
EXP4 extracts prompts only from the training pool so that the robustness study does not leak validation information into prompt construction. The selection pipeline is explicit and staged:
\begin{enumerate}
  ...
  \item keep only the top 35\% quality candidates, implemented as a quality-percentile threshold of 65, which leaves 364 high-quality candidates in the documented experiment run;
  \item greedily select ten prompts that maximize the minimum quaternion angular distance, targeting at least \(30^\circ\).
\end{enumerate}

This prompt set is then used to evaluate the base checkpoint and the fine-tuned single-prompt and multi-prompt checkpoint families inherited from EXP3. The resulting analysis computes prompt-level mean performance, identifies the best and worst prompts, and correlates their outcomes with rotational angles, position in the tomogram and embedding statistics. In the underperforming-prompt follow-up, prompts 1 and 3 emerge as the weak group and prompts 6 and 9 as the strong reference group, which is what makes the later feature-comparison tables possible. The distance-based evaluator itself is kept unchanged from the rest of the UMU benchmark.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l c Y}
    \toprule
    Comparison & Mean \(F_1\) & Observation \\
    \midrule
    Base family & 0.014 & Very poor aggregate performance on the synthetic validation set \\
    Single-prompt fine-tuned family & 0.823 & Strongly and significantly above base \\
    Multi-prompt fine-tuned family & 0.823 & Strongly and significantly above base \\
    Single vs multi family test & -- & No significant difference at aggregate family level, \(p=1.0000\) \\
    \bottomrule
  \end{tabularx}
  \caption{Aggregate comparison in EXP4.}
  \label{tab:exp4-aggregate}
\end{table}

At first sight, Table~\ref{tab:exp4-aggregate} seems to suggest that prompt sensitivity might no longer matter after fine-tuning. That interpretation would be misleading. The family-level averages hide a wide dispersion between prompts.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l Y}
    ...
    Quantity & Value \\
    \midrule
    Global result for \texttt{multi\_inc16} & \(F_1 = 0.840 \pm 0.130\) \\
    Mean precision & 0.912 \\
    Mean recall & 0.809 \\
    Best observed case & Prompt 6 on \texttt{tomo\_rec\_7\_snr1.13}, \(F_1 = 0.959\) \\
    Weakest selected prompt & Prompt 1, mean \(F_1 = 0.599\), mean recall \(= 0.437\) \\
    Strong selected prompts & Prompt 7, mean \(F_1 = 0.915\); prompt 9, mean \(F_1 = 0.913\) \\
    Additional underperforming-prompt analysis & Worst candidate prompt reached \(f1\_mean = 0.4761\) \\
    \bottomrule
  \end{tabularx}
  \caption{Prompt variability observed in EXP4.}
  \label{tab:exp4-prompt-variability}
\end{table}

Table~\ref{tab:exp4-prompt-variability} makes the bottleneck explicit. Even within a strong checkpoint family, some prompts are consistently far better than others. The degradation happens mostly through recall collapse, not through a uniform loss of precision. Therefore, the prompt is not merely a minor nuisance parameter; it is one of the main determinants of whether the detector activates properly.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.86\textwidth]{assets/figures/results/exp4_f1_by_prompt.png}
  \caption{EXP4 prompt-level mean \(F_1\) from the repository underperforming-prompt analysis. Aggregate family averages hide a wide spread between prompts.}
  \label{fig:exp4-prompt-f1}
\end{figure}
...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
    \toprule
    Feature & Correlation with \(F_1\) & \(p\)-value \\
    \midrule
    Rotation about the \(x\)-axis & \(+0.625\) & 0.0534 \\
    Embedding dispersion & \(+0.556\) & 0.0952 \\
    Distance to tomogram center & \(+0.479\) & 0.1616 \\
    Tomogram SNR & \(+0.036\) & 0.9218 \\
    \bottomrule
  \end{tabular}
  \caption{Selected correlations reported in the EXP4 underperforming-prompt analysis.}
  \label{tab:exp4-correlations}
\end{table}

\subsection{Interpretation}
The correlations in Table~\ref{tab:exp4-correlations} should not be overinterpreted, because the sample size is limited. Even so, they reinforce a coherent narrative: prompt orientation and embedding-related descriptors appear more informative than SNR alone. In particular, rotation about the \(x\)-axis, embedding dispersion and distance to the tomogram center all show more signal than the tomogram SNR. Figure~\ref{fig:exp4-prompt-f1} complements that conclusion visually by showing that the spread between prompts is too large to dismiss as noise. This is one of the most important outcomes of the thesis, because it shifts the next research step from \enquote{collect more data} to \enquote{understand and control prompt quality}. The detector is already data-efficient. What it still lacks is a stable conditioning mechanism.

\subsection{Direct summary}
The important lesson of EXP4 is that good averages can hide fragile prompts. After fine-tuning, ProPicker is no longer limited mainly by lack of training data, but by the fact that some prompts activate the detector much better than others. In other words, the current frontier of optimization is not \enquote{more of the same training}, but better prompt selection, prompt ranking and prompt-quality control.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.13\textwidth} >{\raggedright\arraybackslash}p{0.36\textwidth} Y}
    ...
    EXP4 & Good family averages hide large prompt-to-prompt variability; prompt descriptors explain more than SNR alone & The main remaining limit is prompt robustness, not lack of training data \\
    \bottomrule
  \end{tabularx}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.17\textwidth} >{\raggedright\arraybackslash}p{0.37\textwidth} Y}
    ...
    Robustness analysis & Evaluate prompts individually and correlate failures with geometry and embedding descriptors & EXP4 shows that prompt variability survives after fine-tuning and is not explained by SNR alone \\
