...
Usage:
    ...
    python ../../experiments/exp4_ppicker_rotations/scripts/polnet_inference_large.py --checkpoint-type base
    python ../../experiments/exp4_ppicker_rotations/scripts/polnet_inference_large.py --checkpoint-type single --increment 8
    python ../../experiments/exp4_ppicker_rotations/scripts/polnet_inference_large.py --checkpoint-type multi --increment 8
    
    # Run inference with a specific prompt:
    python ../../experiments/exp4_ppicker_rotations/scripts/polnet_inference_large.py --prompt-idx 0
"""
...
RUN_RESULTS_DIR = os.path.join(DEFAULT_RESULTS_BASE, DEFAULT_RUN_NAME)
RUN_PROMPTS_DIR = os.path.join(RUN_RESULTS_DIR, "prompts")
RUN_INFERENCE_DIR = os.path.join(RUN_RESULTS_DIR, "inference")
RUN_TMP_DIR = os.path.join(RUN_RESULTS_DIR, "tmp")

# Fixed checkpoint for this large run
FIXED_CHECKPOINT_TYPE = "multi"
FIXED_INCREMENT = 16
...
def run_inference(checkpoint_type, increment, prompt_idx, prompt_file, force=False):
    ...
    # Results directory
    results_output_dir = os.path.join(RUN_INFERENCE_DIR, result_name)
    ...
    tmp_dir = os.path.join(RUN_TMP_DIR, result_name)
    ...
    for src_dir_name in ["PredictedLabels", "full_segmentation_output"]:
        ...
        possible_srcs = [
            ...
            os.path.join(RUN_RESULTS_DIR, src_dir_name),
        ...
        if not found:
            print(f"    {src_dir_name} not found in expected locations")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None, help="Deprecated: this script is fixed to multi checkpoint")
    parser.add_argument(
        "--increment", type=int, default=None,
        help="Deprecated: this script is fixed to increment 16")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Subdirectory under results/exp4_ppicker_rotations for this run")
    parser.add_argument(
        "--prompts-dir", type=str, default=None,
        help="Directory containing prompt_*.json files (overrides --run-name)")
    parser.add_argument(
    ...
    print("=" * 70)
    print(f" Fixed checkpoint: {FIXED_CHECKPOINT_TYPE}_inc{FIXED_INCREMENT}")
    
    # Resolve run directories
    run_base = args.results_root if args.results_root else DEFAULT_RESULTS_BASE
    run_root = run_base if args.results_root else os.path.join(run_base, args.run_name)
    RUN_RESULTS_DIR = run_root
    RUN_PROMPTS_DIR = args.prompts_dir if args.prompts_dir else os.path.join(RUN_RESULTS_DIR, "prompts")
    RUN_INFERENCE_DIR = os.path.join(RUN_RESULTS_DIR, "inference")
    RUN_TMP_DIR = os.path.join(RUN_RESULTS_DIR, "tmp")

    os.makedirs(RUN_INFERENCE_DIR, exist_ok=True)
    os.makedirs(RUN_TMP_DIR, exist_ok=True)

    # Find prompt files
    prompt_dir = RUN_PROMPTS_DIR
    ...
    if args.prompt_idx is not None:
        ...
        if args.num_prompts <= 0:
            print(" --num-prompts must be > 0")
            sys.exit(1)
        prompt_indices = list(range(min(len(prompt_files), args.num_prompts)))
    
    print(f" Prompts to evaluate: {prompt_indices}")
    print(f" Run root: {RUN_RESULTS_DIR}")
    print(f" Prompts dir: {RUN_PROMPTS_DIR}")
    print(f" Inference dir: {RUN_INFERENCE_DIR}")
    
    # Determine checkpoints to evaluate (fixed for this script)
    if args.checkpoint_type not in (None, FIXED_CHECKPOINT_TYPE):
        print(
            f" Ignoring --checkpoint-type {args.checkpoint_type}. "
            f"Using fixed {FIXED_CHECKPOINT_TYPE}."
        )
    if args.increment not in (None, FIXED_INCREMENT):
        print(
            f" Ignoring --increment {args.increment}. "
            f"Using fixed {FIXED_INCREMENT}."
        )

    checkpoint_configs = [(FIXED_CHECKPOINT_TYPE, FIXED_INCREMENT)]
    ...
    print(f" Results saved to: {RUN_INFERENCE_DIR}")
